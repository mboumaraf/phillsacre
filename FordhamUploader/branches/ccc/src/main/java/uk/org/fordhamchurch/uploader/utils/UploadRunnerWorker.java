...
public class UploadRunnerWorker extends SwingWorker<Void, String>
{
    private static final Log          LOG = LogFactory
	                                          .getLog(UploadRunnerWorker.class);
	...
	_targetFilename = _upload.getOutputFile().getName();
    }
	...
	{
	    LOG.info("Task was cancelled", e);
	}
	catch (Exception e)
	{
	    LOG.error("Could not upload", e);
	...
	finally
	{
	    ...
	    {
		LOG.warn("Could not delete WAV file");
	    }
	    if (null != _mp3 && !_mp3.delete())
	    {
		LOG.warn("Could not delete MP3 file");
	    }
	}
	...
	LOG.debug("Drive: " + drive);

	CDRipper ripper = CDRipper
	        .getRipperForPlatform(drive, progressListener);

	try
	{
	    File rawFile = ripper.rip();

	    LOG.debug("Rip complete, file size is: " + rawFile.length()
		    + " bytes");

	    if (0 == rawFile.length())
	    {
		throw new RuntimeException(
		        "Filesize is zero - problem with CD?");
	    }
    ...
    {
	MP3Encoder encoder = new MP3Encoder(wavFile, _upload.getTitle(),
	        progressListener);
	File mp3 = encoder.encode();

	LOG.debug("Encode complete, encoded file size is " + mp3.length()
	        + " bytes");

	if (mp3.length() == 0)
	{
	    throw new RuntimeException(
		    "Could not convert to MP3 - Filesize is zero - problem with CD?");
	}
	...
	if (_upload.getOutputFile().isFile())
	{
	    _upload.getOutputFile().delete();
	}

	mp3.renameTo(_upload.getOutputFile());
    }
	...
	try
	{
	    ...
	    LOG.info("Creating folder structure: " + folders);

	    sftp.mkdirs(folders);

	    sftp.cd(folders);

	    LOG.info(String.format("Uploading [%s] to [%s]", mp3File.getName(),
		    sftp.pwd() + "/" + _targetFilename));

	    sftp.put(mp3File.getAbsolutePath(), _targetFilename,
		    new UploadProgressListener(progressListener));
	...
	{
	    LOG.error("Could not upload to website", e);
    ...
    {
	FordhamDAO.getInstance().addSermon(_upload.getSpeaker().getId(),
	        _upload.getBook().getId(), _upload.getChapter(),
	        _upload.getDate(), _upload.getTitle(), _targetFilename);
    }
