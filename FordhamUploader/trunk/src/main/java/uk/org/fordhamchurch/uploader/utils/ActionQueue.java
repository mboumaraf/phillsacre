...
/**
 * Action queue designed so that each action can set its own progress to be between 0 and 100, and this will set the
 * total progress accordingly.
 * 
 * @author Phill Sacre
 * @since 14 Jul 2011
 */
public class ActionQueue implements ProgressListener
{
    ...
    {
        int range = 100 / _actions.size(); // e.g. 4 actions will equal range of 25
        float delta = (float) percentage / 100.0f; // The percentage as a float, e.g. 0.5
        int converted = Math.round( range * delta ); // The converted range, e.g. 12

        int base = _executingIndex * range; // The base value of the total percentage, i.e. 25 * 3 = 75
